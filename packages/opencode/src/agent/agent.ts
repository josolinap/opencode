...
export namespace Agent {
  ...
  export async function generate(input: { description: string }) {
    ...
    const mod = await Provider.getModel(def.providerID, def.modelID)
    const system = SystemPrompt.header(def.providerID)
    ...
    const key = String(
      Bun.hash.xxHash32(
        JSON.stringify({
          ...
          p: def.providerID,
          m: def.modelID,
          e: existing.map((i) => i.name).join(","),
        }),
      ),
    )
    const result = await ModelBatcher.call(
      key,
      () =>
        generateObject({
          temperature: 0.3,
