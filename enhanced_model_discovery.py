...
Created: 2025-11-13
Updated: 2025-11-13 - Integrated into main codebase
"""
...
class HuggingFaceModelScanner:
    ...
    async def discover_models(self) -> List[ModelEntry]:
        ...
        # Curated list of high-quality free models
        curated_models = [
            {
...
class GitHubModelScanner:
    ...
    async def discover_models(self) -> List[ModelEntry]:
        ...
        # Curated list of popular model repositories
        curated_repos = [
            {
                'owner': 'ggerganov',
                'repo': 'llama.cpp',
...
class OpenRouterModelScanner:
    ...
    async def discover_models(self) -> List[ModelEntry]:
        """Discover popular free models from OpenRouter"""
        models = []

        # Curated list of high-quality free models from OpenRouter
        curated_models = [
            {
                'id': 'meta-llama/llama-3.2-3b-instruct',
                'name': 'Llama 3.2 3B Instruct',
                'description': 'Meta\'s Llama 3.2 3B instruction-following model',
                'capabilities': ['reasoning', 'text_generation', 'streaming'],
            ...
            },
            {
                ...
                'name': 'Claude 3 Haiku',
                'description': 'Anthropic\'s fast and efficient Claude 3 model',
                'capabilities': ['reasoning', 'text_generation', 'vision', 'multimodal'],
        ...
        for model_data in curated_models:
            ...
            model = ModelEntry(
                ...
                pricing=ModelPricing(
                    cost_per_token=model_data['pricing'],
                    is_free=model_data['pricing'] == 0.0,
                    provider_subsidized=True,
                    tier="free"
                ),
                metadata=ModelMetadata(
                    ...
                    provider='openrouter',
                    description=model_data['description'],
                    homepage_url=f"{self.base_url}/models/{model_id}",
...
class GroqModelScanner:
    ...
    async def discover_models(self) -> List[ModelEntry]:
        """Discover models from Groq"""
        models = []

        # Curated list of Groq models (known for speed)
        curated_models = [
            {
                'id': 'llama2-70b-4096',
                'name': 'Llama 2 70B',
                'description': 'Meta\'s Llama 2 70B model optimized for speed on Groq hardware',
                'capabilities': ['reasoning', 'text_generation', 'code_generation', 'streaming'],
                'context_tokens': 4096,
                'pricing': 0.0
            },
            {
        ...
        for model_data in curated_models:
            ...
            model = ModelEntry(
                ...
                limits=ModelLimits(
                    context_tokens=model_data['context_tokens'],
                    output_tokens=min(model_data['context_tokens'] // 4, 4096),
                    requests_per_minute=30  # Groq rate limits
                ),
                pricing=ModelPricing(
                    cost_per_token=model_data['pricing'],
                    is_free=model_data['pricing'] == 0.0,
...
class TogetherModelScanner:
    """Scanner for Together AI models"""

    def __init__(self, cache: SmartCache):
        self.cache = cache
        self.base_url = "https://together.ai"

    async def discover_models(self) -> List[ModelEntry]:
        """Discover models from Together AI"""
        models = []

        # Curated list of Together AI models
        curated_models = [
            {
                'id': 'togethercomputer/llama-2-70b-chat',
                'name': 'Llama 2 70B Chat',
                'description': 'Meta\'s Llama 2 70B conversational model',
                'capabilities': ['reasoning', 'text_generation', 'code_generation', 'streaming'],
                'context_tokens': 4096,
                'pricing': 0.0
            },
            {
                ...
                'description': 'Meta\'s Llama 2 7B with extended 32K context',
                'capabilities': ['reasoning', 'text_generation', 'code_generation', 'streaming'],
                'context_tokens': 32768,
                'pricing': 0.0
            },
            {
                ...
                'description': 'Meta\'s CodeLlama 34B for code generation and understanding',
                'capabilities': ['reasoning', 'code_generation', 'text_generation'],
            ...
            },
            {
                ...
                'name': 'Llama 2 7B Chat',
                'description': 'Meta\'s Llama 2 7B conversational model',
                'capabilities': ['reasoning', 'text_generation', 'streaming'],
                'context_tokens': 4096,
                'pricing': 0.0
            }
...
class ReplicateModelScanner:
    """Scanner for Replicate models"""
    ...
    async def discover_models(self) -> List[ModelEntry]:
        ...
        # Curated list of Replicate models (focus on free tier)
        curated_models = [
            {
                'id': 'meta/llama-2-70b-chat',
                'name': 'Llama 2 70B Chat (Replicate)',
                'description': 'Meta\'s Llama 2 70B hosted on Replicate',
                'capabilities': ['reasoning', 'text_generation', 'code_generation'],
                'context_tokens': 4096,
                'pricing': 0.0
            },
            {
                ...
                'name': 'Mistral 7B Instruct',
                'description': 'Mistral AI\'s 7B model on Replicate',
                'capabilities': ['reasoning', 'text_generation', 'code_generation'],
                'context_tokens': 4096,
                'pricing': 0.0
            },
            {
                'id': 'replicate/llama-2-13b-chat',
                'name': 'Llama 2 13B Chat',
                'description': 'Meta\'s Llama 2 13B conversational model',
                'capabilities': ['reasoning', 'text_generation', 'streaming'],
                'context_tokens': 4096,
                'pricing': 0.0
            }
        ...
        for model_data in curated_models:
            ...
            # Check cache first
            if not self.cache.should_scan_model(model_id, 'replicate', True):
                continue
            ...
            model = ModelEntry(
                ...
                limits=ModelLimits(
                    context_tokens=model_data['context_tokens'],
                    output_tokens=min(model_data['context_tokens'] // 4, 4096),
                    requests_per_minute=10  # Replicate rate limits
                ),
                pricing=ModelPricing(
                    cost_per_token=model_data['pricing'],
                    is_free=model_data['pricing'] == 0.0,
        ...
        curated_models = [
            {
                'id': 'llama2',
                'name': 'Llama 2',
                'description': 'Meta\'s Llama 2 model running locally via Ollama',
                'capabilities': ['reasoning', 'text_generation'],
                'context_tokens': 4096,
                'pricing': 0.0
            },
            {
                'id': 'mistral',
                'name': 'Mistral',
                'description': 'Mistral AI\'s 7B model running locally',
                'capabilities': ['reasoning', 'text_generation', 'code_generation'],
                'context_tokens': 4096,
                'pricing': 0.0
            },
            ...
            },
            {
                'id': 'vicuna',
                'name': 'Vicuna',
                'description': ' LMSYS Vicuna model based on Llama',
                'capabilities': ['reasoning', 'text_generation'],
                'context_tokens': 2048,
                'pricing': 0.0
            }
...
class EnhancedModelDiscoveryEngine:
    ...
    def _initialize_scanners(self):
        ...
        if not self._scanners:
            self._scanners = {
                'huggingface': HuggingFaceModelScanner(self.cache),
                'github': GitHubModelScanner(self.cache),
                'openrouter': OpenRouterModelScanner(self.cache),
                'groq': GroqModelScanner(self.cache),
                'together': TogetherModelScanner(self.cache),
                'replicate': ReplicateModelScanner(self.cache),
                'ollama': OllamaModelScanner(self.cache),
            }
...
class EnhancedModelDiscoveryModule:
    ...
    def _register_enhanced_sources(self):
        ...
        # Enhanced OpenCode models source
        ...
        self.discovery_engine.register_discovery_source("replicate",
            lambda: self.discovery_engine._scanners['replicate'].discover_models())
    ...
    async def initialize(self) -> None:
        """Initialize the Enhanced Model Discovery Module"""
