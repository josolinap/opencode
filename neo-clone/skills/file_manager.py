...
class FileManagerSkill(BaseSkill):
    ...
    def description(self) -> str:
        return "Manages file operations: reads files, shows directory info, analyzes file contents"
    
    @property
    def example_usage(self) -> str:
        return "Show me information about this file, read a text file, or list directory contents"
    ...
    def execute(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not file_path:
            return {
                "error": "No file path found in request. Please specify a file path.",
                "usage": "Try: 'read /path/to/file.txt' or 'show info about file.py'"
            }
        ...
        try:
            ...
            if any(word in text for word in ["read", "show", "display", "content"]):
                ...
                # Default: provide file info
                return self._file_info(file_path)
                
        except PermissionError:
            return {
                "error": f"Permission denied accessing: {file_path}",
                "suggestion": "Check file/directory permissions or try running with appropriate privileges"
            }
    ...
    def _extract_file_path(self, text: str) -> str:
        ...
        path_patterns = [
            ...
            r'[^\s]+\.(py|js|ts|json|yaml|yml|txt|csv|md|rst|log|ini|cfg|conf)',  # Files with extensions
        ]
        ...
        # If no clear path found, look for words that might be filenames
        words = text.split()
        for word in words:
    ...
    def _read_file(self, file_path: Path) -> Dict[str, Any]:
        ...
        if file_path.is_dir():
            return {
                "type": "directory",
                "path": str(file_path),
                "message": f"{file_path} is a directory. Use 'list' to see contents."
            }
        
        # Check file size
        file_size = file_path.stat().st_size
        
        # Limit file reading for large files
        max_read_size = 50 * 1024  # 50KB limit
        if file_size > max_read_size:
            return {
                ...
                "message": f"File is too large to read entirely ({self._format_size(file_size)}). Reading first part...",
                "content": self._read_partial_file(file_path, max_read_size),
                "suggestion": "Consider using search to find specific content in large files"
            }
        ...
        try:
            ...
            return {
                ...
                "path": str(file_path),
                "size": self._format_size(file_size),
                "message": "Binary file detected. Content cannot be displayed as text.",
                "suggestion": "Use specialized tools to view this file type"
            }
    ...
    def _read_file_by_type(self, file_path: Path) -> str:
        ...
        if suffix == '.json':
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                content = f.read()
                if len(content) > 5000:  # 5KB limit for text files
    ...
    def _search_file(self, file_path: Path, search_term: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "matches": matches,
                "summary": f"Found '{search_term}' in {len(matches)} lines out of {line_count} total lines"
            }
        except Exception as e:
            return {
