...
class OpencodeLLMClient:
    ...
    def _get_api_client(self, provider: str) -> 'APIAdapter':
        ...
        adapters = {
            'ollama': OllamaAdapter(self.cfg, self.session),
            'openai': OpenAIAdapter(self.cfg, self.session), 
            'anthropic': AnthropicAdapter(self.cfg, self.session),
            'api': GenericAPIAdapter(self.cfg, self.session)  # For other providers
        }
        return adapters.get(provider, GenericAPIAdapter(self.cfg, self.session))
