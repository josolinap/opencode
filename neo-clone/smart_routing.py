...
class RoutingContext:
    ...
    def __hash__(self):
        """Make RoutingContext hashable by using key fields"""
        # Create a hash based on the most important fields
        key_str = f"{self.user_input}_{self.urgency}_{self.domain}_{self.complexity_score}_{self.language}"
        return hash(key_str)
    
    def __eq__(self, other):
        """Equality comparison for RoutingContext"""
        if not isinstance(other, RoutingContext):
            return False
        return (
            self.user_input == other.user_input and
            self.urgency == other.urgency and
            self.domain == other.domain and
            self.complexity_score == other.complexity_score and
...
class SmartRouter:
    ...
    def route_request(self, context: RoutingContext) -> RoutingDecision:
        ...
        estimated_cost = self._estimate_cost(selected_model, required_skills, context)
    ...
    def _estimate_cost(self, model_id: str, required_skills: List[str], context: RoutingContext = None) -> float:
        """Estimate cost for routing decision"""
        model = self.models[model_id]
        estimated_tokens = min(len(context.user_input.split()) * 2, model.max_tokens) if context else 100
