...
# Import new analytics and autonomous skills
...
class SelfOptimizingBrain:
    ...
    def __init__(self, config: Optional[Config] = None):
        ...
        # Add skills used tracking to the existing skills
    ...
    def process(self, user_input: str) -> Dict[str, Any]:
        ...
        try:
            # Enhanced intent parsing with optimization
            ...
            response_data["performance_metrics"] = {
                ...
                "optimization_applied": len(self.optimization_history)
            }
            
            # Autonomous optimization checks
    ...
    def parse_intent_with_optimization(self, text: str) -> Dict[str, Any]:
        ...
        # Skill-based intent (enhanced with learning)
        intent_data = self.parse_intent(text)
        
        # Track routing pattern for learning
    ...
    def track_routing_pattern(self, text: str, intent_data: Dict[str, Any]):
        ...
        if intent_data.get("intent") == "skill":
            ...
            if skill_name:
                self.routing_patterns[skill_name].append({
                    "input": text,
                    "timestamp": time.time(),
    ...
    def execute_skill_with_tracking(self, intent_data: Dict[str, Any], user_input: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Determine skill parameters based on input
            ...
            return {
                ...
                "performance_data": {
                    "skill_execution_time": execution_time,
                    "success": True
                }
            }
    ...
    def extract_skill_params(self, skill_name: str, user_input: str) -> Dict[str, Any]:
        ...
        # Enhanced parameter extraction with learning
        params = {}
    ...
    def format_skill_response(self, skill_name: str, skill_result: Dict[str, Any], user_input: str) -> str:
        ...
        if skill_name == "text_analysis":
            ...
            return f"🔍 **Search Results:**\n\n" + "\n".join([
                f"• {result.get('title', 'Untitled')}: {result.get('snippet', 'No description')}"
                for result in results[:3]
            ])
        
        elif skill_name == "analytics_analyzer":
            analysis_type = skill_result.get("analysis_type", "comprehensive")
            return f"📈 **Analytics Report ({analysis_type}):**\n\n" \
                   f"Analysis completed with insights and recommendations."
        
        else:
            return f"✅ {skill_name.title()} completed successfully with results available."
    ...
    def handle_workflow_request(self, user_input: str) -> Dict[str, Any]:
        ...
        if "generate" in user_input.lower() or "create" in user_input.lower():
            ...
            result = self.workflow_generator.execute({
                "workflow_type": workflow_type,
                "automation_level": "full"
            })
            
            workflow = result.get("workflow", {})
            response = f"🔄 **Generated Workflow: {workflow.get('name', 'Custom')}**\n\n"
            ...
            return {
                "response": "🔄 Workflow execution started. Checking available workflows...",
    ...
    def handle_optimization_request(self, user_input: str) -> Dict[str, Any]:
        ...
        response = f"🚀 **System Optimization Report:**\n\n"
    ...
    def handle_model_switch(self, user_input: str) -> Dict[str, Any]:
        ...
        if self.llm_client.switch_model(model_part):
            ...
            return {
                "response": f"✅ Model switched to: {model_part}",
                "model_switch": True,
                "new_model": model_part
            }
        else:
            return {
                "response": f"❌ Failed to switch to model: {model_part}",
                "model_switch": False,
                "error": True
            }
...
Respond helpfully and suggest using skills when appropriate."""
    ...
    def should_optimize(self) -> bool:
        ...
        # Optimize every 10 messages or after 5 minutes
        return (self.session_stats['message_count'] % 10 == 0 or 
                time.time() - self.conversation_history.session_start > 300)
    
    def run_autonomous_optimization(self) -> Dict[str, Any]:
        """Run autonomous optimization routines"""
        optimization_results = {
            "routing_optimization": None,
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            "brain_type": "self_optimizing",
            "current_model": self.current_model,
            "provider": self.config.provider,
            "model_name": self.config.model_name,
            "opencode_model": self.config.opencode_model,
            "is_opencode_available": self.config.opencode_model is not None,
            "model_switches": self.model_switch_count,
            "conversation_stats": {
                **self.session_stats,
                "session_duration": time.time() - self.conversation_history.session_start
            },
            "available_skills": list(self.skills.skills.keys()),
            "skill_count": len(self.skills.skills),
            "config_source": "opencode" if self.config.opencode_model else "local",
            "autonomous_features": {
                "usage_analyzer": True,
                "performance_monitor": True,
                "routing_optimizer": True,
                "dependency_analyzer": True,
                "workflow_generator": True
            },
            "optimization_history_count": len(self.optimization_history),
            "routing_patterns_count": sum(len(patterns) for patterns in self.routing_patterns.values())
        }
    ...
    def generate_analytics_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "routing_analysis": self.routing_optimizer._analyze_current_routing_patterns(),
            "optimization_recommendations": self.routing_optimizer._generate_optimization_recommendations(),
            "workflow_opportunities": self.dependency_analyzer._find_integration_opportunities()
        }
