"""
autonomous_brain_demo.py - Core autonomous brain features demonstration

This demo showcases the core autonomous features without TUI dependencies:
...
def demo_model_integration(brain: SelfOptimizingBrain):
    ...
    for model in model_switches:
        ...
        try:
            ...
            if hasattr(brain.llm_client, 'switch_model'):
                if brain.llm_client.switch_model(model):
                    print(f"   ✅ Successfully switched to {model}")
                    brain.current_model = model
                else:
                    print(f"   ❌ Failed to switch to {model}")
            else:
                print(f"   🔄 Model switching not available in current configuration")
...
• Opencode model integration and switching
• Advanced autonomous intelligence
