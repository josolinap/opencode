# Script: inspect skills package files and runtime module state to diagnose why registry is not present.
import os
import sys
import textwrap
import importlib
...
def safe_import_skills_package():
    # Ensure project root is first to avoid shadowing
...
def inspect_file_contents():
    ...
    for fname in sorted(os.listdir(SKILLS_DIR)):
        ...
        print(f"--- {fname} ---")
        # print small header and first 200 chars
...
def inspect_skill_modules_imported():
    ...
    for m in mods:
        ...
        # show if module tried to access skills.DEFAULT_REGISTRY etc by scanning source if available
        try:
...
def try_invoking_registry(pkg):
    ...
    # heuristic: find any object with list_skills and execute_skill methods
...
def main():
    ...
    if forced:
        ...
        # After loading, see if forced module has registry
