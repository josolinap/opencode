...
class Config(BaseModel):
    # Neo-Clone configuration (maintained for backward compatibility)
...
def find_opencode_config() -> Optional[str]:
    ...
    # Check common user config directories
...
def get_available_opencode_models() -> List[str]:
    ...
    try:
        # Try to get models using Opencode CLI
        ...
        if result.returncode == 0:
            # Parse output - should be in provider/model format
            models = []
...
def translate_opencode_model_to_neo(opencode_model: str) -> tuple[str, str]:
    ...
    # Map common Opencode providers to Neo-Clone providers
    provider_mapping = {
        "openai": "api",
        "anthropic": "api", 
        "google": "api",
...
def load_config(path: Optional[str] = None) -> Config:
    ...
    """
    try:
        # 1. Check for Opencode integration
        ...
        # 2. Load traditional config if specified
        neo_config_data = {}
        ...
        # 3. Get model from Opencode if available
        opencode_model = None
        ...
        # 4. Build configuration with Opencode integration
        env = {
        ...
        # Override with traditional config if present
        env.update(neo_config_data)
        
        # Create config object
        config = Config(**env)
        
        # If Opencode model is available, translate and use it
        if opencode_model:
...
# Backward compatibility - keep the original load_config function name
