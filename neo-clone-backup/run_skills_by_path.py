# Force-load skills package by path and run discovery + smoke tests
import os
import sys
import logging
...
def safe_load_module_from_path(module_name: str, path: str):
    ...
    try:
        ...
        # Insert into sys.modules under the provided name to make relative imports inside module work
...
def discover_and_run(module):
    ...
    try:
        ...
        if hasattr(module, "DEFAULT_REGISTRY"):
            ...
            # look for any object named SkillRegistry instance
            for name in ("SkillRegistry",):
                if hasattr(module, name):
                    candidate = getattr(module, name)
                    logging.info(f"Found attribute {name} in module (likely class): {candidate}")
            # leave registry None for now

        # Try calling discover_skills in multiple ways
        if hasattr(module, "discover_skills"):
            ...
            try:
                ...
                # Try calling with registry if available
        ...
        # If we don't have a registry yet, try to find one in module globals
        if registry is None:
        ...
        # If still no registry, attempt to find an instance by type detection
        if registry is None:
            for vname, val in vars(module).items():
                # Heuristic: registry-like objects have methods list_skills and execute_skill/register_skill
        ...
        if registry is None:
            ...
            # Try loading skill modules by file path under skills/ directory
            ...
            # Try again to locate registry in module globals
            for attr in ("DEFAULT_REGISTRY", "registry", "skill_registry", "SKILL_REGISTRY"):
                if hasattr(module, attr):
                    registry = getattr(module, attr)
        ...
        # Final check: if registry available, list skills
        if registry is not None:
...
def main():
    try:
        ...
        # Load under a unique module name to avoid shadowing by other 'skills' modules
        module_name = "neo_clone_skills_forced"
        module = safe_load_module_from_path(module_name, SKILLS_INIT)

        # Show sys.modules entries related to skills
